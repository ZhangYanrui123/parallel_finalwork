...
void gpu_queue(bool** M1, bool** M2) {
    gpu_selector selector;
    queue q(selector);
    device my_device = q.get_device();
    std::cout << "Device: " << my_device.get_info<info::device::name>() << std::endl;
    //共享内存
    bool** d_M1 = malloc_shared<bool*>(COL,q);
    bool** d_M2 = malloc_shared<bool*>(COL,q);
    memcpy(d_M1, M1, sizeof(bool) * COL);
    memcpy(d_M2, M2, sizeof(bool) * COL);
        ...
        for (int j = COL-1; j >=0; j--) {
            ...
            if (M2[i][j]) {
                ...
                if(M1[j][j]){
                    ...
                    {
                        int k = idx[0] + j; // 等同于for(int j=i+1; j<n; j++)
                        d_M2[i][k] = d_M1[j][k] != d_M2[i][k];
